...
1. Add a service method and a struct in the blog.thrift file for deleting a blog post.
...
1. Add a service method and a struct for deleting a blog post in the blog.thrift file, as we did in [1. Implement server-side](#1-implement-server-side).
2. Add a service method, `deleteBlogPost()`.
