...
1. Add a method and a struct and an exception in the blog.thrift file, as we did in [1. Implement server-side](#1-implement-server-side).
2. Add a method in the `BlogClient.java` to retrieve a single post.
...
1. Add a method and structs in blog.thrift file just like the server.
2. Add a method in the `BlogClient.java` to retrieve a list of posts.
  ```java filename=BlogClient.java
  import example.armeria.blog.thrift.ListBlogPostsRequest;
  import example.armeria.blog.thrift.ListBlogPostsResponse;
...
Here, we've implemented service methods and client methods to retrieve blog posts. We have also added a decorator. See [Decorating a client](/docs/client-decorator/) for more information. Next, we'll implement a method for [updating a blog post](/tutorials/thrift/blog/implement-update) and add an exception handler.
