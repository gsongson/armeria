...
As the first step of the tutorial, we'll write a Thrift server that serve a `hello` service. 

<Tip>

Please refer to [Running a Thrift service](/docs/server-thrift) for creating Thrift server.

</Tip>

## 1. Create a Thrift file

Create a file, `blog.thrift` inside the directory, `{project_root}/src/main/thrift` each server and client.
...
In this step, we've defined a thrift file for our service and generated Java code. And then, we've created and added a Thrift service to a server. 

Next, we'll implement a minimal client and made a call to the server to [test run](/tutorials/thrift/blog/run-service).
